(function(){
  ...
  async function guardOnLoad(){
    try{
      ...
      if(!v.ok){
        show();
        window.dispatchEvent(new CustomEvent('kovalucky-license-change',{detail:{state:'invalid', data:v}}));
        return;
      }
      const d = v.data || v;
      if(!d.deviceId || d.deviceId !== deviceId){
        show();
        window.dispatchEvent(new CustomEvent('kovalucky-license-change',{detail:{state:'invalid', data:d}}));
        return;
      }
      updateStatus(d);
      window.dispatchEvent(new CustomEvent('kovalucky-license-change',{detail:{state:'valid', data:d}}));
